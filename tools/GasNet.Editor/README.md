...
- **校验 = 写出 → 真加载器读回的往返**。每次修改都全量执行（`CatalogDocument.Validate`），
  加载器报什么编辑器显示什么——游戏读不进的文件不会无声地存出去（保存前确认）。
...
1. **加载档案**：填游戏托管 DLL 路径。仓库内运行会自动预填 GodotDemo 构建产物
   ...
   不持有游戏 DLL 的文件句柄——编辑器开着也能重新编译游戏；档案会检测 DLL 重建
   （时间戳/长度变化）并在界面上提示重新加载。
2. **打开目录**：效果列表按 JSON 出现顺序显示，左侧可新建/克隆/删除效果、按名称过滤、
   查看已注册标签树。加载器拒绝重复的效果键（`Duplicate effect name '…'`），打开即报；
   有未保存修改时打开新文件会先确认。
3. **编辑**：右侧改字段，顶部校验横幅实时显示往返结果。标签输入框带自动补全
   （datalist）；原文件里没有的新标签会软提醒——可能是拼写错误，运行时模型会静默
   注册任何新标签。重命名效果会提示：保存只更新目录内的键，目录外的旧引用（游戏代码
   里的 `catalog.Get("…")` 等）不会自动更新。支持文档级撤销/重做（快照栈）。
   效果底部有只读的"JSON 预览"：该效果保存时的写出形态。
4. **反查**：左栏"引用查询"按标签（层级匹配：查 `Stun` 命中 `Stun.Blocked`）或属性
   （`SetTypeName.FieldName`）列出引用位置，点击结果跳到对应效果。
5. **保存**：写回原路径；往返校验仍有错误时先确认——允许保存 WIP，但不会无声覆盖。

界面分区：左栏 = 档案 / 目录文件 / 标签树 / 引用查询；右栏 = 校验横幅 / 效果编辑器 /
GasNetLog 日志面板（核心库的 Warn/Error 进这里，控制台同时保留）。支持明暗主题切换。
...
| `CatalogDocument.cs` | 编辑中的文档：有序效果列表 + 脏标记 + 撤销/重做（深拷贝快照栈）+ 克隆 + Open/Save/Validate |
| `CatalogSearch.cs` | 反查扫描：标签/属性被哪些效果的哪些字段引用；"新标签"提醒的数据源 |
| `EditorProfile.cs` | 档案：反射加载游戏 DLL，发现属性集与可引用类型；构建 `GasNetDataLoadOptions`；DLL 重建检测 |
| `Ui.cs` | 标签文本 ↔ 容器；InvariantCulture 的数字/枚举解析 |
| `EditorLogBuffer.cs` | 线程安全日志环形缓冲（60 行） |
| `Components/Pages/Editor.razor` | 主页面：三步流程、效果列表与过滤、标签树、引用查询、校验横幅、撤销/重做、日志；所有编辑汇入 `Refresh()` |
| `Components/EffectEditor.razor` | 单个效果全部字段：基本 / 修饰符 / 执行计算与自定义应用条件 / 标签 / 标签需求 / 驱散与免疫 / 堆叠 / 授予能力；底部 JSON 预览 |
...
1. **不得在编辑器里写任何 JSON 序列化/格式校验代码**。新字段的读写只经 `GasNet.Data`；
   那边还不支持就先改那边。想加校验规则，加到 `GasNetDataLoader`——手写在编辑器里
   必然与加载器漂移。编辑器允许 UX 级启发提示（新标签软提醒、保存前确认），但它们只
   提醒、不改变格式语义，也不替代加载器报错。
...
3. **数字解析一律 InvariantCulture**（`Ui.F` / `Ui.I` / `Ui.Enum`），别用依赖当前
   区域设置的 `float.Parse`。
4. **所有编辑事件汇入 `Editor.razor` 的 `Refresh()`**（撤销快照 + 标脏 + 往返校验 +
   标签树更新），新 UI 控件不要各自刷新。撤销快照依赖 `CatalogDocument.CloneDef` 的
   深拷贝：给定义加可变引用类型字段时记得在那里补克隆，否则撤销/克隆会共享引用。
...
4. 若新字段是可变引用类型：`CatalogDocument.CloneDef` 补深拷贝（否则撤销/克隆共享引用），
   `CatalogSearch` 视情况补反查扫描。

**加一类可引用的代码类型**（下拉框支持新的 calculation 基类）：
`EditorProfile.LoadAssembly` 的类型分派 → `BuildOptions` 的注册循环 →
`EffectEditor.razor` 的下拉框。

**编辑器自身的行为回归**：`tests/GasNet.Editor.Tests`（文档往返/改名跟踪/克隆/撤销、
档案加载与过期检测、反查扫描）——改 `CatalogDocument`/`EditorProfile` 时先跑它。
...
- 无与运行中游戏的连接、无多文档同时打开。
- 标签不做预注册硬校验：核心库是运行时注册模型，JSON 里的新标签首用即自动注册；
  标签树/补全只是"已注册标签"的展示，新标签的拼写风险由软提醒兜着，不是硬校验。
